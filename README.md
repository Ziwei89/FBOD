...
**<font color=red>注意：</font>论文中使用的数据集(监控视频飞鸟目标数据)，部分不能公开。我们已经采集可以公开的数据集FBD-SV-2024，已经公开！数据集下载地址及相关处理脚本请访问https://github.com/Ziwei89/FBD-SV-2024_github  **
...
python train_AP50.py \
        ...
        --Add_name=20230822
```
训练时，程序将在TrainFramework/目录下创建一个logs/five/384_672/RGB_relatedatten_cspdarknet53_concat_aa_20230822/的目录，该目录会保存一些训练模型。同时，在该目录下会创建一张loss.jpg的图像用于记录训练过程的loss，训练到30个epoch后，还会创建一张ap50.jpg的图像用于记录后续模型的AP50性能指标。  
